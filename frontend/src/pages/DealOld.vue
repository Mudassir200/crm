    ...
    <Resizer side="right" class="flex flex-col justify-between border-l">
      ...
      >
        ...
        >
          ...
          <!-- <div v-else-if="section.name == 'properties_section'" class="properties-area">
            <div
              v-if="dealProperties?.loading && dealProperties?.data?.length == 0"
              class="flex min-h-20 flex-1 items-center justify-center gap-3 text-base text-ink-gray-4"
            >
              <LoadingIndicator class="h-4 w-4" />
              <span>{{ __('Loading...') }}</span>
            ...
            <div
              v-else-if="dealProperties?.data?.length"
              v-for="(property, i) in dealProperties.data"
              :key="property.name"
            >
            <div class="px-2 pb-2.5" :class="[i == 0 ? 'pt-5' : 'pt-2.5']">
              <Section :opened="property.opened">
                  <template #header="{ opened, toggle }">
                    <div
                      class="flex cursor-pointer items-center justify-between gap-2 pr-1 text-base leading-5 text-ink-gray-7"
                    >
            ...
            <div
                v-if="i != dealProperties.data.length - 1"
                class="mx-2 h-px border-t border-outline-gray-modals"
              />
          </div>
            <div
...
function getParsedSections(_sections) {
  _sections.forEach((section) => {
    ...
    if (section.name == 'properties_section') return
