              ...
              <div v-for="(field, i) in group" :key="i" :id="'filter-list' + i" class="mb-4 sm:mb-3">
                <div v-if="isMobileView" class="flex flex-col gap-2">
                  ...
                  <div id="fieldname" class="w-full">
                    <Autocomplete :value="field[3].fieldname" :options="filterableFields.data"
                      @change="(e) => updateFilter(e, g, i)" :placeholder="__('First Name')" />
                  </div>
                  <div id="operator">
                    <FormControl type="select" v-model="field[1]" @change="(e) => updateOperator(e, field, g, i)"
                      :options="getOperators(field[3].fieldtype, field[3].fieldname)" :placeholder="__('Equals')" />
                  </div>
                  <div id="value" class="w-full">
                    <component :is="getValueControl(field)" v-model="field[2]" @change.stop="(v) => updateValue(v, field)"
                ...
                <div v-else class="flex items-center justify-between gap-2">
                  <div class="flex items-center gap-2">
                    ...
                    <div id="operator">
                      <FormControl type="select" v-model="field[1]" @change="(e) => updateOperator(e, field, g, i)" :options="getOperators(field[3].fieldtype, field[3].fieldname)" :placeholder="__('Equals')" />
                    </div>
                    <div id="value" class="!min-w-[140px]">
                      <component :is="getValueControl(field)" v-model="field[2]" @change="(v) => updateValue(v, field)"
                        :placeholder="__('John Doe')" />
                    </div>
                  </div>
                  <Button class="flex" variant="ghost" icon="x" @click="removeFilter(i)" />
...
function convertToGroupedFilters(data, allFilters) {
  ...
  let initialGroup = allFilters.map((group) => {
    return group.map((filter) => {
      let value = filter
      let field = data.find((f) => f.fieldname === value[0])
      filter[3] = field
      if (field?.fieldtype === 'Check') {
        filter[2] = value[2] ? 'Yes' : 'No'
      }
      filter[1] = oppositeOperatorMap[value[1]]
      return filter
    })
  })
...
function getValueControl(f) {
  ...
  if (operator == 'is') {
    ...
    return h(DateRangePicker, { value: f[2], iconLeft: '' })
...
function setfilter(data, group = "new") {
  ...
  groups[group].push([
    data.fieldname,
    getDefaultOperator(data.fieldtype),
...
function updateOperator(event, filter, group, i) {
  ...
  filter[1] = newOperatorValue
  if (!isSameTypeOperator(oldOperatorValue, newOperatorValue)) {
    filter[2] = getDefaultValue(filter[3])
  }
